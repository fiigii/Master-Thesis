...
We describe pushdown CFA algorithms on ANF style lambda calculus.
\[
\tag{expressions}
\begin{aligned}
\label{eq:bnf}
  ...
  case State(KReturnComplete(v), env, localStack, a, memory) =>
      ...
      for {
        GlobalFrame(returnPoint, oldStack, savedEnv, newGlobalAddress)
            <- newMemory.getGlobalFrames(a) //get the stack frame
      } yield {
...
case State(KFuncCallComplete(funcRef, args), env, localStack, a, memory) =>
    val newMemory = memory.copy(state)
    for {
        f <- newMemory.getValue(funcRef) //get function objects
        if isCallable(f)
...
New call (functions called as constructors) is another function invocation form in JavaScript, which generates a new and empty object at the ``new call site'', passes the object as \verb|this| argument, and return the generated object as computing result of the new call. In JsCFA, we implement the ``return the generated object'' by a trick, which puts a extra local continuation object \verb|KUseValue(obj)| into the new \verb|localStack|.
