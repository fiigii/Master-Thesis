...
\subsection{Store-Widening and Complexity of \textit{h}-CFA}
\label{sub:swh}
Store-Widening is also employed in \textit{h}-CFA for value and continuation store both.
However,
\[
O(
(\overbrace{n}^{|Exp|} + \overbrace{n}^{|\widetilde{Env}|} + \overbrace{n^n}^{|\widetilde{History}|} + \overbrace{n^n}^{|\widetilde{KAddr}|}) \times \overbrace{n^2}^{|\widetilde{Store}|} \times \overbrace{n^2}^{|\widetilde{KStore}|}
)
\]
Although the number of reachable states of $CESK^H$ machine is still exponential, in practice the performance of \textit{h}-CFA is better than AAC for most cases.
