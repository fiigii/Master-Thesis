...
\end{lstlisting}
In \verb|compose-same|, local variable \verb|f| is referring to closures and called twice.
In actual runtime, these two call sites always invoke same function.
However 0-CFA or \textit{k}-CFA with not enough context length may compose different closures for these two call sites when \verb|compose-same| is called multiple times and serval different closures flow into \verb|f|.
