...
\[
\widetilde{a^H_k{}_1} = (fib, \{fib\})
\]
\[
\widetilde{a^H_k{}_2} = (fib, \{fib, res1, res2\})
\]
\[
\begin{aligned}
\label{eq:show-stack}
...
\[
\widetilde{a^H_k{}_3} = (fib, \{fib, res1, res2, res3, res4\})
\]
...
\widetilde{a^H_k{}_2} \mapsto \{ {}& (res3, (let\ (res4\ (-\ n\ 2)) \dots), \widetilde{env_2}, \tilde{h_2}, \widetilde{a^H_k{}_1}), {} \\
                                 {}& (res3, (let\ (res4\ (-\ n\ 2)) \dots), \widetilde{env_a}, \tilde{h_a}, \widetilde{a^H_k{}_2}), {} \\
                                   & (res3, (let\ (res4\ (-\ n\ 2)) \dots), \widetilde{env_b}, \tilde{h_b}, \widetilde{a^H_k{}_3}) \}
\end{aligned}
\]

The merging expresses a fact that the invocation of ``fib'' at point (fib res2) may be made by (a (fib 10), (fib res2), or (fib res4). Moreover, the second frame in above illustration has the ``next'' pointer $\widetilde{a^H_k{}_2}$ that refers to itself. This cycle makes the continuation store no longer stack-like, but a graph. Concurrently, $\widetilde{a^H_k{}_2}$ is not the only circular pointer, and the abstract continuation in $\widetilde{a^H_k{}_3}$ also contains cycles because $\widetilde{a^H_k{}_3}$ allocated by another recursive call.
...
\[
\widetilde{a^H_k{}_5} = (fib, \{fib, a, res1, res2\})
\]
